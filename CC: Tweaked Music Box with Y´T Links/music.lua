-- ============================ --
-- 	 Edited by Domenik_YT1  	--
-- 								--
--        Original Code:        --
-- 			   from 	   		--
--https://pastebin.com/Rc1PCzLH --
-- ============================ --
...
-- Helper: shorten text so it fits within maxLen characters
...
-- Try to find a "length"/"duration" style field on a song table,
-- since different API versions may name this differently.
local function getLength(song)
	if song == nil then return nil end
	return song.length or song.duration or song.duration_seconds or song.song_length
end

function redrawScreen()
	-- Keep any connected monitors in sync whenever the UI would redraw,
	-- regardless of whether we're currently waiting on text input.
	drawMonitors()
...
local function buildMonitorLines()
	local lines = {}

	if now_playing ~= nil then
		table.insert(lines, { text = now_playing.name, color = colors.white })
		table.insert(lines, { text = now_playing.artist, color = colors.lightGray })

		local length = formatTime(getLength(now_playing))
		if length ~= nil then
			table.insert(lines, { text = length, color = colors.lightGray })
		end
	else
		table.insert(lines, { text = "Not playing", color = colors.lightGray })
	end

	if is_loading == true then
		table.insert(lines, { text = "Loading...", color = colors.gray })
	elseif is_error == true then
		table.insert(lines, { text = "Network error", color = colors.red })
	end
...
function drawMonitors()
	...
	local lines = buildMonitorLines()
	local numLines = #lines

	local longestLine = 0
	for _, line in ipairs(lines) do
		longestLine = math.max(longestLine, #line.text)
	end

	for _, mon in ipairs(monitors) do
		local scales = { 5, 4, 3, 2, 1.5, 1, 0.5 }
		local chosenScale = 0.5
		local w, h = mon.getSize()

		for _, scale in ipairs(scales) do
			mon.setTextScale(scale)
			w, h = mon.getSize()
			if h >= numLines + 1 and w >= longestLine + 2 then
				chosenScale = scale
				break
			end
		end
		mon.setTextScale(chosenScale)
		w, h = mon.getSize()

		mon.setBackgroundColor(colors.black)
		mon.clear()

		local startY = math.floor((h - numLines) / 2) + 1
		if startY < 1 then startY = 1 end

		for i, line in ipairs(lines) do
			local text = truncate(line.text, w - 2)
			local x = math.floor((w - #text) / 2) + 1
			if x < 1 then x = 1 end

			mon.setTextColor(line.color)
